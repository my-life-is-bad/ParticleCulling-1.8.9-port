		...
		for (String className : Configuration.ignoredParticles) {
			try {
				...
				if (particleClass.isAssignableFrom(clazz)) //returns true if the vanilla particle class is equal to or a super class of clazz
					...
					LOGGER.warn("{} is not a particle class or does not extend the vanilla particle class!", className);
			}
			catch (ClassNotFoundException e) {
				LOGGER.warn("Could not find particle class {}. If the mod is not installed, this can be ignored.", className);
			}
		}
	}

	public static boolean isParticleIgnored(Class<?> particleClass) {
		for (Class<?> ignoredParticleClass : Configuration.ignoredParticleClasses) {
			if (ignoredParticleClass.isAssignableFrom(particleClass)) //returns true if ignoredParticleClass is equal to or a super class of the given particle class
				return true;
		}
